...
namespace Rex.WorkFlow.Web.Services.Impls
{
    public class SysRoleService : ISysRoleService
    {
        ...
        public async Task<RoleIndexViewModel> GetListAsync(RoleIndexSearch search, bool isSearchPage = false)
        {
            RoleIndexViewModel dto = new RoleIndexViewModel();
            if (isSearchPage)
            {
                dto.Page = await _databaseFixture.Db.SysRole.GetPageAsync(search);
            } else
            {
                var dbsystems = await _databaseFixture.Db.SysSystem.FindAllAsync(m => m.IsDel == 0);
                dto.Systems = dbsystems.Select(m => new SelectListItem()
                {
                    Text = m.SystemName,
