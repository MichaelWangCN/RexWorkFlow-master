...
namespace Rex.WorkFlow.Web.Services.Impls
{
    ...
    public class SysResourceService : ISysResourceService
    {
        ...
        public async Task<IList<SelectListItem>> GetSystemItems(long systemId)
        {
            var dbsys = await _databaseFixture.Db.SysSystem.FindAllAsync(m => m.IsDel == 0);
            //拉取系统 
            IList<SelectListItem> sysResourceTrees  = dbsys.Select(m => new SelectListItem()
            {
                Value = m.SystemId.ToString(),
                Text = m.SystemName,
                Selected = m.SystemId == systemId
            }).ToList();
            return sysResourceTrees;
        }

        public async Task<IList<SysResourceTree>> GetResourceTreeAsync(long systemId)
        {
            string _where = string.Empty;
            if (systemId > 0)
            {
                _where = $" AND SystemId = { systemId }";
            }
            string sql = $"SELECT * FROM sys_resource WHERE 1 = 1" + _where + " ORDER BY Sort ASC"; // IsButton = 0
            var resourceData = await _databaseFixture.Db.SysResource.QueryAsync(sql);

            IList<SysResource> resourceList = resourceData.ToList();
            IList<SysResourceTree> rList = resourceList.MapToList<SysResource, SysResourceTree>();
            foreach (SysResourceTree srt in rList)
            {
                srt.Buttons = resourceList.Where(m => m.ParentId == srt.ResourceId && m.IsButton == 1)
                    .Select(m => new IdAndValue()
                    {
                        Id = m.ResourceId,
                        Value = m.ResourceName
            ...
            return rList.Where(m => m.IsButton == 0).ToList();
        }
